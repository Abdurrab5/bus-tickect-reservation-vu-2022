...
$query="SELECT * FROM schedule where bus_no='$bus_no'   ";
		$result= mysqli_query($link,$query);
		   while( $row=mysqli_fetch_assoc($result)){
 ...
 $query="INSERT into bookings(username,s_id,bus_no,seat_no,dep_from,dep_to,amount,dep_date,dep_time,status)VALUES";
    $query.="('$username','$s_id','$bus_no','$seat_no','$dep_from','$dep_to','$amount','$dep_date','$dep_time','pending')";
 $result= mysqli_query($link, $query);

		   
				  
    $query="update seats set status=1  where seat_id='$id'";
 ...
 alert("Bus update successfuly.");
       ...
       redirect_to("route.php");
				   ...
				   $query="update seats set status=0  where seat_id='$id'";
    
 $result= mysqli_query($link, $query);
  alert("Bus update successfuly.");
       
        redirect_to("viewseat.php");
...
<?php	
 ...
 <?Php  
							 if($status==1){ ?>
<td rowspan="2"><a href="addseat.php" role="button" class="btn btn-block btn-danger">

Seat No:<?Php echo $seat_no;?></a>;</td>
    
                           
					<?Php	}else{     ?>
	<td rowspan="2"><a href="addseat.php" role="button" class="btn btn-block btn-success">
